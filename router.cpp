...
std::string Router::route(std::string source_name, std::string target_name) {
  ...
  while (!label_queue.empty()) {
    ...
    if (deleted_labels.count(curr_label.label_id) == 1 || shortest_path_tree.count(curr_label_id) == 1) {
      continue;
    }
    ...
    for (const NodeID& adj_node_id : graph_.at(curr_label_id)) {
      ...
      if (dist_to_neighbor <= curr_label.state_of_charge) {
        labels.emplace_back(
            Label(
              adj_node_id, label_id++, curr_label.total_weight + direct_weight_to_neighbor,
              0, curr_label.state_of_charge - dist_to_neighbor, curr_label_id));
      }
      ...
      if (curr_label.state_of_charge < MAX_CHARGE) {
        ...
        labels.emplace_back(
            Label(
              adj_node_id, label_id++, curr_label.total_weight + direct_weight_to_neighbor + addtl_charge_time,
              // curr_label.charge_time + addtl_charge_time, MAX_CHARGE - dist_to_neighbor, curr_label_id));
              addtl_charge_time, MAX_CHARGE - dist_to_neighbor, curr_label_id));
      }
      // 3. Only charge enough to get to neighbor.
      if (curr_label.state_of_charge < MAX_CHARGE && curr_label.state_of_charge < dist_to_neighbor) {
        Weight addtl_charge_time = time_to_partial_charge(
            curr_label.state_of_charge, dist_to_neighbor, curr_row.rate);
        labels.emplace_back(
            Label(
              adj_node_id, label_id++, curr_label.total_weight + direct_weight_to_neighbor + addtl_charge_time,
...
std::string Router::build_result_string(
    const NodeToLabelMap& shortest_path_tree, NodeID source_node_id, NodeID target_node_id) {
